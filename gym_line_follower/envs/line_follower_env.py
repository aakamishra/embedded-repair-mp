...
class LineFollowerEnv(gym.Env):
    ...
    def __init__(self, gui=True, nb_cam_pts=8, sub_steps=10, sim_time_step=1 / 250,
                 ...
                 track=None, track_render_params=None, hardware_label=np.array([1., 1., 1., 1.])):
        """
        Create environment.
        ...
        hardware_label = np.random.choice([1.0, 0.9, 0.8, 0.0], 4, p=[0.4, 0.4, 0.05, 0.15])
        self.gui = gui
        ...
        filename = "/home/ec2-user/embedded-repair-mp/gym_line_follower/line_follower_bot.py"
        ...
        # self.gnn_model = TemporalLSTMGCN(input_dim=50, hidden_dim=256, output_dim=4).to("cuda")
        # # Path to your saved checkpoint
        # checkpoint_path = '/home/ec2-user/embedded-repair-mp/saved_gnn_models3/model_epoch_50.pt'  # Replace with your checkpoint path
    ...
    def reset(self):
        ...
        if len(obsv) < 1:
            ...
            #     obsv = np.array([0.0, 0.0] * self.nb_cam_pts + [1]*4)
            return obsv
    ...
    def step(self, action):
        ...
        # new_type_array[non_zero_indices] = np.concatenate([np.zeros(4), self.follower_bot.hardware_node_array]) * new_type_array[non_zero_indices]
        ...
        #             edge_index=torch.tensor(np.vstack(edge_index))).to("cuda")
        #     self.preds = self.gnn_model(data.x, data.edge_index, data.batch).detach().cpu().numpy()
